        ...
        {isPlaying ? (
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between text-slate-400">
               <div className="flex items-center gap-2"><Clock size={14} className="text-billar-neon" /><span className="text-sm font-mono tracking-tighter">00:00:00</span></div>
...
// --- Sale / Consumo Modal ---
const SaleModal = ({ isOpen, table, products, onClose, onConfirm, user }) => {
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center p-6">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card w-full max-w-4xl h-[80vh] flex flex-col relative z-[2001] overflow-hidden bg-billar-dark shadow-2xl border-white/5">
        <div className="p-6 border-b border-white/5 flex justify-between items-center">
          <h2 className="text-2xl font-black italic uppercase tracking-tighter">Nueva <span className="text-billar-neon">Venta / Consumo</span></h2>
          <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-full transition-all"><X /></button>
        </div>
        <div className="flex-1 flex overflow-hidden">
          {/* Product Picker */}
          <div className="flex-1 p-6 overflow-y-auto border-r border-white/5 bg-black/20">
            {!table && <div className="mb-4"><label className="text-[10px] font-black text-slate-500 uppercase block mb-1">Nombre del Cliente</label><input type="text" className="w-full neon-input py-2" value={clienteNombre} onChange={e => setClienteNombre(e.target.value)} placeholder="Ej: Jonathan Lopez" /></div>}
            <div className="grid grid-cols-2 gap-3">
              {products.map(p => (
                <button key={p.id} onClick={() => addToCart(p)} className="glass-card p-4 text-left hover:bg-white/5 active:scale-95 transition-all group">
                  <p className="font-bold text-sm group-hover:text-billar-neon transition-all">{p.nombre}</p>
          ...
          {/* Cart */}
          <div className="w-80 p-6 flex flex-col bg-white/[0.02]">
            <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-6">Resumen de Cuenta</h3>
            <div className="flex-1 space-y-3 overflow-y-auto pr-2">
              {cart.map(item => (
                <div key={item.id} className="flex justify-between items-center">
                  <div className="min-w-0 flex-1"><p className="font-bold text-xs truncate">{item.nombre}</p><p className="text-[10px] font-black text-slate-500">{item.q} x ${item.precio}</p></div>
                  <div className="flex items-center gap-2"><button onClick={() => removeFromCart(item.id)} className="w-6 h-6 rounded bg-white/5 flex items-center justify-center hover:bg-red-500/20 text-red-500"><Minus size={12} /></button><button onClick={() => addToCart(item)} className="w-6 h-6 rounded bg-white/5 flex items-center justify-center hover:bg-emerald-500/20 text-emerald-500"><Plus size={12} /></button></div>
                </div>
              ))}
              {cart.length === 0 && <div className="h-40 flex items-center justify-center text-slate-700 text-[10px] font-black uppercase text-center">El carrito está vacío</div>}
            </div>
            <div className="mt-6 pt-6 border-t border-white/5">
              <div className="flex justify-between items-end mb-6"><p className="text-slate-500 text-xs font-black uppercase">TOTAL</p><p className="text-3xl font-black text-emerald-400">${total}</p></div>
              <button disabled={cart.length === 0} onClick={() => onConfirm(table, cart, clienteNombre)} className="w-full neon-button py-4 font-black italic uppercase disabled:opacity-20 transition-all">Generar Pedido</button>
            </div>
          </div>
        </div>
      </motion.div>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-billar-dark flex flex-col selection:bg-billar-neon">
              ...
              {currentView === 'dashboard' && (
                <div className="p-8">
                  <div className="flex justify-between items-end mb-8">
                    ...
                    <button onClick={() => { setSelectedTable(null); setSaleModalOpen(true); }} className="neon-button flex items-center gap-2"><ShoppingCart size={18} /> NUEVA VENTA</button>
                  </div>
                  <StatsGrid tables={tables} />
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {tables.map(t => <TableCard key={t.id} table={t} onStartMatch={id => { setSelectedTable(id); setMatchModalOpen(true); }} onFinishMatch={handleFinishMatch} onAddConsumo={t => { setSelectedTable(t); setSaleModalOpen(true); }} />)}
                  </div>
                </div>
              )}
      ...
      <SaleModal isOpen={saleModalOpen} table={selectedTable && selectedTable.id ? selectedTable : null} products={products} user={user} onClose={() => setSaleModalOpen(false)} onConfirm={handleOrderConfirm} />
    </div>
  );
}

// Reuse TVView from previous implementation
...
// --- Sub-Views --- (Login, Register, History, Music as simple placeholders or functional as before)
